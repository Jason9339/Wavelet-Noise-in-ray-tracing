...
void WaveletNoise::generateNoiseTile2D() {
    ...
    std::vector<float> temp1(num_elements); // 工作陣列 1
    std::vector<float> temp2(num_elements); // 工作陣列 2
    ...
    // Process along X axis: r_initial -> temp1
    std::copy(r_initial.begin(), r_initial.end(), temp1.begin());
    for (int iy = 0; iy < n; ++iy) {
        int base = iy * n;
        
        // Extract row (X-axis line)
        for (int ix = 0; ix < n; ++ix) {
            line_in[ix] = r_initial[base + ix];
        }
    ...
    for (int ix = 0; ix < n; ++ix) {
        // Extract column (Y-axis line)
        for (int iy = 0; iy < n; ++iy) {
            line_in[iy] = temp1[ix + iy * n];
        }
        ...
        // Store result back
        for (int iy = 0; iy < n; ++iy) {
            temp2[ix + iy * n] = line_out[iy];
        }
    }
    // temp2 現在包含了 r_ds_us_xy

    // Step 4: Subtract to get N = R - R_ds_us_xy
    for (int i = 0; i < num_elements; ++i) {
    ...
    for (int ix = 0; ix < n; ++ix) {
        for (int iy = 0; iy < n; ++iy) {
            shifted[ix + iy * n] = noiseCoefficients[Mod(ix + offset, n) +
                                                     Mod(iy + offset, n) * n];
        }
    }
...
float WaveletNoise::evaluate2D(const float p[2]) const {
    ...
    // Sum contributions from 9 neighboring coefficients
    for (f[1] = -1; f[1] <= 1; ++f[1]) {
        for (f[0] = -1; f[0] <= 1; ++f[0]) {
...
void WaveletNoise::generateNoiseTile3D() {
    ...
    std::vector<float> r_initial(num_elements);
    std::vector<float> r_ds_us_xyz(num_elements);
    ...
    saveIntermediateData(r_initial, n, DebugStep::R_INITIAL);
    ...
    // Process along X, Y, Z axes
    std::vector<float>* from = &r_initial;
    std::vector<float>* to = &temp;
    
    // X axis
    for (int iy = 0; iy < n; ++iy) for (int iz = 0; iz < n; ++iz) {
        for (int ix = 0; ix < n; ++ix) line_in[ix] = (*from)[ix + iy * n + iz * n * n];
        downsample1D(line_in, line_ds, n, 1);
        upsample1D(line_ds, line_out, n, 1);
        for (int ix = 0; ix < n; ++ix) (*to)[ix + iy * n + iz * n * n] = line_out[ix];
    }
    saveIntermediateData(*to, n, DebugStep::AFTER_X_FILTER);
    std::swap(from, to);

    // Y axis
    for (int ix = 0; ix < n; ++ix) for (int iz = 0; iz < n; ++iz) {
        for (int iy = 0; iy < n; ++iy) line_in[iy] = (*from)[ix + iy * n + iz * n * n];
        downsample1D(line_in, line_ds, n, 1);
        upsample1D(line_ds, line_out, n, 1);
        for (int iy = 0; iy < n; ++iy) (*to)[ix + iy * n + iz * n * n] = line_out[iy];
    }
    saveIntermediateData(*to, n, DebugStep::AFTER_Y_FILTER);
    std::swap(from, to);

    // Z axis
    for (int ix = 0; ix < n; ++ix) for (int iy = 0; iy < n; ++iy) {
        for (int iz = 0; iz < n; ++iz) line_in[iz] = (*from)[ix + iy * n + iz * n * n];
        downsample1D(line_in, line_ds, n, 1);
        upsample1D(line_ds, line_out, n, 1);
        for (int iz = 0; iz < n; ++iz) (*to)[ix + iy * n + iz * n * n] = line_out[iz];
    }
    saveIntermediateData(*to, n, DebugStep::AFTER_Z_FILTER);
    // At this point, 'to' contains the final low-pass filtered R_ds_us_xyz
    r_ds_us_xyz = *to;

    // Step 4: Subtract to get N = R - R_downsample_upsample
    for (int i = 0; i < num_elements; ++i) {
        noiseCoefficients[i] = r_initial[i] - r_ds_us_xyz[i];
    }
    ...
    for (int ix = 0; ix < n; ++ix) for (int iy = 0; iy < n; ++iy) for (int iz = 0; iz < n; ++iz) {
        shifted[ix + iy * n + iz * n * n] = noiseCoefficients[Mod(ix + offset, n) +
                                                              Mod(iy + offset, n) * n +
                                                              Mod(iz + offset, n) * n * n];
    }
